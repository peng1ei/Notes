...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         dim_in_par = length(varargin); % length Êý×é³¤¶È£¨¼´ÐÐÊý»òÁÐÊýÖÐµÄ½Ï´óÖµ£©
         if (nargin - dim_in_par)~=1    % ~= ²»µÈÓÚ
            error('Wrong parameters');
         elseif rem(dim_in_par,2) == 1  % rem ÇóÓà
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         if isempty(R)
            ...
            [L N]=size(R);  % L number of bands (channels) % size ·µ»ØÐÐÁÐÊý
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         if snr_input==0,
            r_m = mean(R,2);  % ¶ÔÓÚ¾ØÕó£¬mean(A,2)¾ÍÊÇ°üº¬Ã¿Ò»ÐÐµÄÆ½¾ùÖµµÄÁÐÏòÁ¿
            R_m = repmat(r_m,[1 N]); % mean of each band %À©Õ¹º¯Êý£¬½«¾ØÕóA¸´ÖÆm¡Án¿é£¬¼´BÓÉm¡Án¿éAÆ½ÆÌ¶ø³É
            R_o = R - R_m;           % data with zero-mean 
            
			%µÃµ½×î´óµÄp¸öÌØÕ÷Öµ¼°Æä¶ÔÓ¦µÄ×î´óÌØÕ÷ÐÐÏòÁ¿ºÍÌØÕ÷ÁÐÏòÁ¿
			[Ud,Sd,Vd] = svds(R_o*R_o'/N,p);  % computes the p-projection matrix % ÆæÒìÖµ·Ö½â£¨Õý½»¾ØÕó·Ö½â·¨£©
            x_p =  Ud' * R_o;                 % project the zero-mean data onto p-subspace£¬Ud' ×ªÖÃ
            
            SNR = estimate_snr(R,r_m,x_p); % ¹À¼ÆÐÅÔë±È
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         if SNR < SNR_th,   
                ...
                y =  x./ repmat( sum( x .* repmat(u,[1 N]) ) ,[d 1]);%Èç¹ûa¡¢bËÑË÷ÊÇ¾ØÕó£¬a./b¾ÍÊÇa¡¢bÖÐ¶ÔÓ¦µÄÃ¿¸öÔªËØÏà³ý£¬µÃµ½Ò»¸öÐÂµÄ¾ØÕó
...
function snr_est = estimate_snr(R,r_m,x)
         ...
         P_y = sum(R(:).^2)/N;    % .^2 ¾ØÕó¸öÃ¿¸öÔªËØ¶ÁÆ½·½
